...
func (client Client) loadActor(config Config, identifier string) (streams.Document, error) {
	...
	if idType == IdentifierTypeUsername {
		...
		if actor := client.loadActor_WebFinger(config, identifier); actor.NotNil() {
	...
	if actor := client.loadActor_ActivityStreams(config, identifier); actor.NotNil() {
		log.Trace().Str("loc", location).Msg("Found via ActivityStream")
		return actor, nil
	}

	// 3. Try RSS/Atom/JSONFeed/MicroFormats
	if actor := client.loadActor_Feed(config, identifier); actor.NotNil() {
